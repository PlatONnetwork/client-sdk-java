...
public class StakingScenario extends Scenario {
    ...
    public void executeScenario() throws Exception {
        //初始化账户余额
        transfer();
        BigInteger stakingBalance = web3j.platonGetBalance(stakingCredentials.getAddress(chainId), DefaultBlockParameterName.LATEST).send().getBalance();
        BigInteger delegateBalance = web3j.platonGetBalance(delegateCredentials.getAddress(chainId), DefaultBlockParameterName.LATEST).send().getBalance();
    ...
    public TransactionResponse unDelegate(BigInteger stakingBlockNum) throws Exception {
        BigDecimal stakingAmount = Convert.toVon("500000", Unit.ATP);
    ...
    public CallResponse<Delegation> getDelegateInfo(BigInteger stakingBlockNum) throws Exception {
    	CallResponse<Delegation> baseResponse = delegateContract.getDelegateInfo(nodeId, delegateCredentials.getAddress(chainId), stakingBlockNum).send();
        return baseResponse;
    }


    public CallResponse<List<DelegationIdInfo>> getRelatedListByDelAddr() throws Exception {
    	CallResponse<List<DelegationIdInfo>> baseResponse = delegateContract.getRelatedListByDelAddr(delegateCredentials.getAddress(chainId)).send();
        return baseResponse;
    }
    ...
    public TransactionResponse staking() throws Exception {
        ...
        BigDecimal stakingAmount = Convert.toVon("5000000", Unit.ATP).add(BigDecimal.valueOf(1L));
    ...
    public void transfer() throws Exception {
        Transfer.sendFunds(web3j, superCredentials, chainId, stakingCredentials.getAddress(chainId), transferValue, Unit.ATP).send();
        Transfer.sendFunds(web3j, superCredentials, chainId, delegateCredentials.getAddress(chainId), transferValue, Unit.ATP).send();
    }
