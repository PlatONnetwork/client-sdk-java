...
public class RestrictingScenario extends Scenario {
	...
	public void executeScenario() throws Exception {
		//初始化账户余额
		transfer();
		BigInteger restrictingBalance = web3j.platonGetBalance(restrictingSendCredentials.getAddress(chainId), DefaultBlockParameterName.LATEST).send().getBalance();
	...
	public CallResponse<RestrictingItem> getRestrictingPlanInfo() throws Exception {
		CallResponse<RestrictingItem> baseResponse = restrictingPlanContract.getRestrictingInfo(restrictingRecvCredentials.getAddress(chainId)).send();
		return baseResponse;
	}
    ...
    public TransactionResponse createRestrictingPlan() throws Exception {
        ...
        PlatonSendTransaction platonSendTransaction = restrictingPlanContract.createRestrictingPlanReturnTransaction(restrictingRecvCredentials.getAddress(chainId), restrictingPlans).send();
        TransactionResponse baseResponse = restrictingPlanContract.getTransactionResponse(platonSendTransaction).send();
        return baseResponse;
    }

	public void transfer() throws Exception {
		Transfer.sendFunds(web3j, superCredentials, chainId, restrictingSendCredentials.getAddress(chainId), transferValue, Unit.LAT).send();
	}
}
